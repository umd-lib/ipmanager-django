...
known_users = {
    'BW': {'first_name': 'Ben', 'last_name': 'Wallberg', 'username': 'wallberg'},
    'KD': {'first_name': 'Kate', 'last_name': 'Dohe', 'username': 'katedohe'},
}
...
def split_notes(apps, schema_editor):
    ...
    for group in group_model.objects.all():
        for old_note in group.notes.split('; '):
            ...
            note = note_model(
                created=timestamp, user=user, group=group, content=content.strip()
            )
            note.save()

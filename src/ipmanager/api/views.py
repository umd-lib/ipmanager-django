...
def build_checks_result(ip_address, group, request):
    return {
        '@id': request.build_absolute_uri(reverse('check'))
        + '?'
        + urlencode({'group': group.key, 'ip': ip_address}),
            ...
            return JsonResponse(
                {
                    ...
                    'ip': ip,
                    'checks': [
                        build_checks_result(ip, group, request) for group in all_groups
                    ],
                }
            )
        else:

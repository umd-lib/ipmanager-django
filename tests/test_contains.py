...
def test_A_includes_B():
    ...
    ip = '10.204.145.0'
    relation1 = Relation(
        subject=group1, object=group2, relation=Relation.RelationType.INCLUSION
    )
    relation1.save()
...
def test_A_excludes_B():
    ...
    ip = '10.204.145.0'
    relation1 = Relation(
        subject=group1, object=group2, relation=Relation.RelationType.EXCLUSION
    )
    relation1.save()
...
def test_A_includes_B_B_includes_C():
    ...
    relation1.save()
    relation2 = Relation(
        subject=group2, object=group3, relation=Relation.RelationType.INCLUSION
    )
    relation2.save()
    ip = ip3.value
    assert ip in group1
...
def test_A_excludes_B_B_excludes_C():
    ...
    relation1.save()
    relation2 = Relation(
        subject=group2, object=group3, relation=Relation.RelationType.EXCLUSION
    )
    relation2.save()
    ip = ip3.value
    assert ip not in group1
...
def test_excludes():
    ...
    relation_include = Relation(
        subject=group1, object=group3, relation=Relation.RelationType.INCLUSION
    )
...
def test_excludes_issue():
    ...
    rel1 = Relation(
        subject=includes_single_ip,
        object=single_ip,
        relation=Relation.RelationType.INCLUSION,
    )
    rel1.save()
    rel2 = Relation(
        subject=group, object=world, relation=Relation.RelationType.INCLUSION
    )
    rel2.save()
    rel3 = Relation(
        subject=group,
        object=includes_single_ip,
...
def test_nested_inclusion():
    ...
    relation4in1 = Relation(
        subject=group1, object=group4, relation=Relation.RelationType.INCLUSION
    )
    relation4in1.save()
...
def test_excluded_farther_down_chain():
    ...
    relation_um_in_films = Relation(
        subject=films_group, object=um_group, relation=Relation.RelationType.INCLUSION
    )
    relation_um_in_films.save()
    relation_usmai_not_in_um = Relation(
        subject=um_group, object=usmai_group, relation=Relation.RelationType.EXCLUSION
    )
...
def test_exclusion_with_inclusion():
    ...
    rel1 = Relation(
        subject=groupA, object=groupB, relation=Relation.RelationType.EXCLUSION
    )
    rel1.save()
    rel2 = Relation(
        subject=groupB, object=groupC, relation=Relation.RelationType.INCLUSION
    )
    rel2.save()
